    ...
    <div class="col-xs-12 hl-left">
         ...
         <?php echo heading($titre,'3');
           $attributes = array('class' => '');
                ...
                echo form_open('scolarite/ecrire_rapport_absences_groupe',$attributes);
        ...
        <table id="rounded-corner" summary="cycle">
            <thead>
                <tr>
                    <th></th>
                    <?php
                    $matricule = array_unique($infoAbsences['matriculeEtudiant']);// groupe qui contient des variable de tout les groupes mais distinct
                    for($i=0 ;$i<count($infoAbsences['matriculeEtudiant']) ;$i++)
                    {
                        if(isset($matricule[$i]))
                        {          
                            echo '<th id="col" class="rounded">'.$matricule[$i].'</th>';
                        }
                    }
            ...
            <tbody id="listeClasse">
                ...
                for($i=0 ; $i<count($infoAbsences['date']) ; $i++)
                {
                    ...
                    $sum = 0;
                    for($j=0 ; $j <count($infoAbsences['matriculeEtudiant']) ;$j++)
                    {
                        if(isset($matricule[$j]))
                        {
