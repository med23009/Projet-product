...
class Directeur_etudes extends CI_Controller {
    ...
    function __construct() 
    {
     ...
     $this->load->library("Pdf");
        ...
        if( !in_array('directeur_etudes', $this->session->userdata('profil')))
                redirect();
    ...
    {
        $this->load->view('directeur_etudes/index');
    }

    public function mot_de_passe() 
    {
        $data['errorMessage'] = '';
        $this->load->view('directeur_etudes/modifier_mot_de_passe', $data);
    }
    ...
    function selection_semestre_module_a_valider() 
    {
        ...
        $this->load->view('directeur_etudes/choix_periode', $data);
    }
    ...
    function consulter_etudiant($matriculeEtudiant) 
    {
        ...
        $this->load->view("directeur_etudes/consulter_info_personnelle_etudiant", $informations_etudiant);
    }
        ...
        if ($this->chef_departement_modele->get_old_password($this->session->userdata('login')) == $this->input->post('old_password')) 
        {
            ...
            if($passwordIsValid == TRUE)
            {
                if ($this->input->post('new_password') == $this->input->post('confirmed_password')) 
                {
                    ...
                    $this->load->view('directeur_etudes/modification_confirme',$data);
                } 
                else 
                {
   ...
   // $this->load->view("recherche_parametree", $data);
    $this->load->view("directeur_etudes/choix_semestre_bulltin",$data);
}
public function voir_bulletin_nouvel() 
    {
       ...
       /* if($semestre%2==1){
        ...
        print_r($moyenne);
        $data = array('moyenne'=>$moyenne,'passage'=>$passage,'code'=>$code,'infoE'=>$dataE,'nc' => $moduleNc,'info' => $infoEtudiant, 'semestre' => $semRes, 'modules' => $moduleRes, 'annee' => $annee,'numSem'=>$semestre);
        $this->load->view('directeur_etudes/bulltin', $data);
    ...
    function choix_langue_attestation_d()
    {
        ...
        $this->load->view("directeur_etudes/choix_langue_attestation_d", $data);
    }
    ...
    function consulter_attestation_d($langue, $id) 
    {
      ...
      // print_r($informations_attest);
        ...
        $this->load->view("directeur_etudes/consulter_info_attestation_d", $informations_attest);
    } 
      function afficher_stat()
    {
        ...
        $this->load->view("directeur_etudes/afficher_stat", $data);       
     ...
     function stat_param()
    {
        ...
        $this->load->view("directeur_etudes/stat_param", $data);       
    ...
    function planning_exam(){
           ...
           $this->load->view("directeur_etudes/planning_exam",$data);      
       }
       ...
       // $groupes=array('listeH'=>$listeH,'groupe'=>$groupe);
        ...
        function planning_examens(){
          ...
          // print_r($data);
           ...
           $this->load->view("directeur_etudes/planning_examens",$data);      
       }
...
$data=array( 'employe'=>$employe,'local' => $info_salle['idLocal'],
        ...
        $this->load->view("directeur_etudes/emplois_du_temps.php",$data);
    }
...
$data=array( 'employe'=>$employe,'local' => $info_salle['idLocal'],
        ...
        //$this->loadData();
                       // $data=array('idProgramme'=>$idProgramme,'semestre'=>$semestre,'listeCours'=>$listeCours);
        $this->load->view("directeur_etudes/emplois_du_temps_date.php",$data);
    }
 ...
 $info_salle = $this->scolarite_modele->recuperer_salle();
				$data = array('sigleCours' => $data_sigle_cours['sigleCours'],
                    ...
                    'semestre' => $data_session_courante['semestre'][0],
                     ...
                     'employe'=>$employe);
                $this->load->view("directeur_etudes/choisir_annee_horaire_emplois", $data);
            }
        }
    }
   ...
   public function loadData()
         ...
         public function ElementsPlanning()
	{
        ...
        echo(json_encode($chevauchement));
         ...
         function modification_note_acce() 
	{
    ...
    {     
            ...
            $this->load->view("directeur_etudes/chef_pass_note", $data);
        ...
        echo(json_encode($info));   
         ...
         function infos_etudiants(){
                      $data_session_courante = $this->scolarite_modele->get_session_courante();
                       ...
                       'semestre' => $data_session_courante['semestre'][0]);
           $this->load->view("directeur_etudes/liste_etudiants",$data);      
       }
       
        public function ElementsModif_note()
	{
		$loadType=$_POST['loadType'];
		$loadId=$_POST['loadId'];
                $semestre=$_POST['semestre'];
		...
		$result=$this->scolarite_modele->getInfoElements_etudiant($loadType,$loadId,$semestre,$matricule);
             //   $this->db->insert("r_events", array('test'=>"SELECT distinct p.sigle,m.titre FROM `planetudes` p,module m,unite u where p.sigle=m.sigle and u.sigle=m.sigleunite and u.semestre=$semestre and u.idProgramme='".$loadId."' and p.sigle not in(select sigle from infomodule_element)  "));
         ...
         public function Element_note()
	{
		...
		$result=$this->scolarite_modele->getInfoElement_note($loadType,$loadId,$evaluation,$matricule,$sigle);
             //   $this->db->insert("r_events", array('test'=>"SELECT distinct p.sigle,m.titre FROM `planetudes` p,module m,unite u where p.sigle=m.sigle and u.sigle=m.sigleunite and u.semestre=$semestre and u.idProgramme='".$loadId."' and p.sigle not in(select sigle from infomodule_element)  "));
		...
		if($loadType=="note_element"){
                     if($result->num_rows() > 0){
			foreach($result->result() as $list){
				$HTML="<input class='form-control'type='text' id='note' value='".$list->note."'/><input type='hidden' id='annee' value='".$list->annee."'/><input type='hidden' id='semestre' value='".$list->semestre."'/>";
			}
                }}
		...
		echo $HTML;
         ...
         public function ajouter_modif_note_temp()
	{
		...
		$result=$this->scolarite_modele->ajouter_modif_note_temp($semestre,$sigle,$matriculeEtudiant,$note,$session,$annee);
             //   $this->db->insert("r_events", array('test'=>"SELECT distinct p.sigle,m.titre FROM `planetudes` p,module m,unite u where p.sigle=m.sigle and u.sigle=m.sigleunite and u.semestre=$semestre and u.idProgramme='".$loadId."' and p.sigle not in(select sigle from infomodule_element)  "));
        }
        ...
        function liste_modif_note() 
	{
		...
		$data = NULL;
            ...
            $this->load->view("directeur_etudes/liste_modif_note",$data);
    		
	
	}
        function historique_modification_note() 
	{
		...
		$data = NULL;
            ...
            $this->load->view("directeur_etudes/historique_modification_note",$data);
       ...
       // $groupes=array('listeH'=>$listeH,'groupe'=>$groupe);
        ...
        function info_historique_modification_note(){
            $groupe['data'] = $this->scolarite_modele->info_historique_modification_note();
       // $groupes=array('listeH'=>$listeH,'groupe'=>$groupe);
        echo(json_encode($groupe));   
       }
}
