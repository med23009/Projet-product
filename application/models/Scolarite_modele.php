       ...
       // $nombreGroupe = count($tableauGroupe);
        ...
        if($filiere=='MPG'){
         ...
         if($filiere=='GC'){
            ...
            print_r($tableSigle);
        $nombreSigle=count($tableSigle); } 
//add by team scolarite
$tableSigle=$this->get_module_prg($filiere);
  $nombreSigle=count($tableSigle);
    for ($i = 0; $i < $nombreEtudiant; $i++) {
         $query = $this->db->query("SELECT DISTINCT `matriculeEtudiant` FROM `dossieretudiant` WHERE `matriculeEtudiant`='$tableauMatricule[$i]' and `idProgramme`<> 'TC'");
           //for($s=0;$s<$nombreSigle;$s++){
//         $this->db->where(array('matriculeEtudiant' => $tableauMatricule[$i], 'annee' => $anneeCourante, 'semestre' => $semestreCourant, 'sigle' =>$tableSigle[$s]));
  //         $query1 = $this->db->get('planetudes');}
            if ($query->num_rows() > 0 ) {
          $query = $this->db->query("DELETE FROM `dossieretudiant` WHERE `matriculeEtudiant`='$tableauMatricule[$i]' and `idProgramme`<> 'TC'");
//                echo $anneeCourante;
                //$this->db->where(array('matriculeEtudiant' => $tableauMatricule[$i],'annee' => $anneeCourante));
 for($s = 0; $s < $nombreSigle; $s++){     
            $this->db->where(array('matriculeEtudiant' => $tableauMatricule[$i], 'annee' => $anneeCourante, 'semestre' => $semestreCourant, 'sigle' =>$tableSigle[$s]));
    $query1 = $this->db->get('planetudes');}
               ...
               // $groupe = $this->get_groupe($post['annee'], $post['session'], $post['sigle']);
                ...
                $data['valide'][$compteursValide] = 'étudiant ' . $tableauMatricule[$i] . ' est bien était reorienter';
                $compteursValide++;
                }
             else {
                $data['message'][$compteurMessage] = '�tudiant ' . $tableauMatricule[$i] . ' ne  pas orienter';
                $compteurMessage++;
            }
        }
 ...
 function get_module_prg($idprogramme){
        $sig=array();
        $str="select m.sigle from unite u join module m on(m.sigleunite=u.sigle)  where u.idProgramme='$idprogramme' and u.semestre=2";
    ...
    function get_module_irt(){
        $sig=array();
        $str="SELECT * FROM `infomodule_element` WHERE `semestre`=2 and (`Filiere`='TC' or `Filiere`='DSI')";
       $query=$this->db->query($str);
       if($query->num_rows()>0){
           foreach($query->result_array() as $row){
               $sig[]=$row['sigle'];
    ...
    function get_module_gm(){
        $sig=array();
        $str="SELECT * FROM `infomodule_element` WHERE `semestre`=2 and (`Filiere`='TC' or `Filiere`='RSS')";
       $query=$this->db->query($str);
       if($query->num_rows()>0){
           foreach($query->result_array() as $row){
               $sig[]=$row['sigle'];
    ...
    function get_module_ge(){
        $sig=array();
        $str="SELECT * FROM `infomodule_element` WHERE `semestre`=2 and (`Filiere`='TC' or `Filiere`='CNM')";
       $query=$this->db->query($str);
       if($query->num_rows()>0){
           foreach($query->result_array() as $row){
               $sig[]=$row['sigle'];
    ...
    function get_module_gc_he(){
        $sig=array();
        $str="SELECT * FROM `infomodule_element` WHERE `semestre`=2 and (`Filiere`='TC' or `Filiere`='GC')";
       $query=$this->db->query($str);
       if($query->num_rows()>0){
           foreach($query->result_array() as $row){
               $sig[]=$row['sigle'];
    ...
    function get_module_mpg(){
        $sig=array();
        $str="SELECT * FROM `infomodule_element` WHERE `semestre`=2 and (`Filiere`='TC' or `Filiere`='MPG')";
       $query=$this->db->query($str);
       if($query->num_rows()>0){
           foreach($query->result_array() as $row){
               $sig[]=$row['sigle'];
    ...
    function get_module_sid(){
        $sig=array();
        $str="SELECT * FROM `infomodule_element` WHERE `semestre`=2 and (`Filiere`='TC' or `Filiere`='SID')";
       $query=$this->db->query($str);
       if($query->num_rows()>0){
           foreach($query->result_array() as $row){
               $sig[]=$row['sigle'];
...
//echo $this->db->last_query();
        if ($query->num_rows() > 0) {
            ...
            if ($row['maxMat'] != 0)
                ...
                $a = $anneeUniv % 100;
                 //add by znb 11-11-2020
                $matricule = $a * 1000 +201;
            }
        } else { // Insertion du premier etudiant de l'an�e en cours
            $a = $anneeUniv % 100;
            //add by znb 11-11-2020
    ...
    function get_informations_etudiant($matricule) {
     ...
     if(isset($row['infoBac'])){
            ...
            $data['infoBac'] = $row['infoBac'];
               ...
               if(isset($row['autreDiplome'])){
            $data['autreDiplome'] = $row['autreDiplome'];}
            else{
            $data['autreDiplome'] = '';    
            }
                   if(isset($row['commentaireAdmission'])){   
            $data['commentaireAdmission'] = $row['commentaireAdmission'];
                   }else{
                         $data['commentaireAdmission'] ='';
                       
                   }
                      if(isset($row['sessionNormale'])){   
...
// added by Hafedh 22/10/2015
    ...
    function recuperer_liste_etudiants_conseil($sigle, $annee, $semestre, $typeRapport, $session = 'N',$idGroupe='') {
       ...
       // $this->db->join('anonymat a ', 'p.matriculeEtudiant=a.matriculeEtudiant and p.annee=a.annee and p.semestre=a.semestre ');
 $this->db->join('listeetudiants l','`p`.`matriculeEtudiant`=`l`.`matriculeEtudiant`');
          ...
          $this->db->where('idEtablissement', $idEtab);
            ...
            if ($query->num_rows() > 0) {
              ...
              $data['nomEtablissement'] = '';
                $data['programme_etudiant'][] =array('idProgramme'=>'');    
            }
...
select evaluation.idEvaluation from  evaluation  NATURAL JOIN moduleevaluation  where sigle='" . $sigle . "' ) ";
    ...
    function get_informationsAtt_d($matricule) {
        ...
        if ($query->num_rows() > 0) {
            ...
            //if($semestre_reel %2==0)
            {
                $q = 'select M.sigle as sigleModule, M.nbCredits , U.credits, U.sigle as sigleUnite, U.semestre, M.sigle,M.titre as titreModule, M.volumeCM, M.volumeTD, U.titre as titreUnite,U.pole from module M,planetudes p, unite U,pole po where M.sigleunite=U.sigle and M.sigle=p.sigle and mod(U. semestre,2)=0 and p.annee=' . ($annee + 1) . ' and p.semestre=1 and p.matriculeetudiant=' . $matricule . ' and po.id=U.pole order by po.ord,M.sigle asc';
                //$q="select M.sigle as sigleModule, M.nbCredits , U.credits, U.sigle as sigleUnite, U.semestre, M.titre as titreModule, M.volumeCM, M.volumeTD, U.titre as titreUnite from module M, unite U where M.sigleunite=U.sigle and semestre=".$semestre." and U.idProgramme='".$row['idProgramme']."' order by U.sigle asc";
               // echo $q; 
                $q = $this->db->query($q);
                $data['maqSemPair'] = $q->result_array();
                // $q="select M.sigle as sigleModule, M.nbCredits , U.credits, U.sigle as sigleUnite, U.semestre, M.titre as titreModule, M.volumeCM, M.volumeTD, U.titre as titreUnite from module M, unite U where M.sigleunite=U.sigle and semestre=".($semestre-1)." and U.idProgramme='".$row['idProgramme']."' order by U.sigle asc";
                $q = 'select M.sigle as sigleModule, M.nbCredits , U.credits, U.sigle as sigleUnite, U.semestre,M.sigle, M.titre as titreModule, M.volumeCM, M.volumeTD, U.titre as titreUnite,U.pole  from module M,planetudes p, unite U,pole po where M.sigleunite=U.sigle and M.sigle=p.sigle and mod(U. semestre,2)=1 and p.annee=' . ($annee) . ' and p.semestre=3 and p.matriculeetudiant=' . $matricule . ' and po.id=U.pole order by po.ord,M.sigle asc';
                // echo $q;
                $q = $this->db->query($q);
                $data['maqSemImpair'] = $q->result_array();
                // Cheikh recuperer le nombrre d'elements, volume CM, volume TD par module et semestre
...
// debut modif PV
   function get_etudiant_anne_semestre($annee = 2015, $semestre = 3, $idProgramme, $semEtude, $session, $tri) {// A revoir pour que �a marche avec les autrs ann�es
        $etudiants = array();
        $query = NULL;
       // echo $annee.' '.$semestre.' '.$idProgramme.' '.$semEtude.' '.$session.' '.$tri;
        $anneeRat = $annee;
       /* if($semEtude%2==0){
            $semEtude=1;
        }
        ...
        if ($session == 1) {
            ...
            $this->db->where(array('p.annee' => $annee, 'p.semestre' => $semestre,'d.idProgramme' => $idProgramme,'u.semestre' => $semEtude));
               //echo $idProgramme;
            $this->db->select('d.matriculeEtudiant');
            $this->db->distinct();
            $this->db->from('dossieretudiant d');
            $this->db->join('planetudes p', 'd.matriculeEtudiant=p.matriculeEtudiant');
            $this->db->join('module m', 'p.sigle=m.sigle');
            $this->db->join('unite u', 'u.sigle=m.sigleunite');
...
r.`semestre` = n.`semestre` and r.annee=(2001+floor(n.annee/100)) and r.matriculeEtudiant = " . $matriculeEtudiant . " and r.semestre =" .
    ...
    function getEtudiants_Niveau($idProgramme, $annee, $niveau) {
         ...
         */
        $this->db->where(array('annee' => $annee, 'niveau' => $niveau, 'idProgramme' => $idProgramme));
        $this->db->distinct();
...
// debut modif optimisation 
    ...
    function getInfoBulletinEtudiant_liste($matriules) {
        ...
        $sql = 'select e.*,p.nom as programme,p.idprogramme as idprogramme  from etudiant e, dossieretudiant d,programme p where d.idProgramme= p.idProgramme and e.matriculeetudiant=d.matriculeetudiant and  e.matriculeetudiant in (' . $matriules . ') order by matriculeEtudiant';
        // echo $sql;
        $infos = array();
        $res = $this->db->query($sql);
        ...
        if ($res->num_rows() > 0) {
            ...
            foreach ($res->result_array() as $row) {
                ...
                $infos['programme'][] = $row['programme'];
                   $infos['idprogramme'][] = $row['idprogramme'];
            }
        }
        //  print_r($)
...
r.`semestre` = n.`semestre` and r.annee=(2001+floor(n.annee/100)) and r.matriculeEtudiant in  (" . $matricules . ") and r.semestre =" .
    ...
    $info = array();
    if(empty($matricules) ||  empty($annee) || empty($semestre)) // add by Med BAkar 24-09-2021
    return $info;
    ...
    function get_niveau() {
        $result = array();
        $sql = 'select distinct niveau from niveau_inscrits ';
        $req = $this->db->query($sql);
        $row = $req->result_array();
        foreach ($row as $value) {
            $result[] = $value['niveau'];
...
r.`semestre` = n.`semestre` and r.annee=(2001+floor(n.annee/100)) and r.matriculeEtudiant = " . $matriculeEtudiant . " and r.semestre =" .
    ...
    function info_bulltin($matriculeEtudiant) {
        ...
        $str = "SELECT distinct count(d.`matriculeEtudiant`) as nb  FROM `dossieretudiant` d WHERE d.`matriculeEtudiant`<=" . $matriculeEtudiant . " and d.`matriculeEtudiant` like '" . $rest . "%'"
 ."  union all SELECT distinct count(d.`matriculeEtudiant`) as nb  FROM `dossieretudiant` d WHERE d.`matriculeEtudiant`<=" . $matriculeEtudiant .
 " and d.`matriculeEtudiant` like '" . $rest . "%' and d.idProgramme = (select idProgramme from dossieretudiant where matriculeEtudiant=" . $matriculeEtudiant .
" and idProgramme!='TC' )";
//remove by team scolarite 19/02/23
 //" and annee=(SELECT MAX(annee) from dossieretudiant WHERE matriculeEtudiant=". $matriculeEtudiant ."
       // echo $str ;
       $info = NULL;
        $query = $this->db->query($str);
        if ($query->num_rows() > 0) {
    ...
    function getInfoElements($loadType, $loadId, $annee, $semestre, $type, $matriculeEmploye) {
        ...
        if ($loadType == "infomodule_element" or $loadType == "infomodule_elementMF" or $loadType == "infomodule_elementMN" or $loadType == "infomodule_elementRT" or $loadType == "infomodule_elementLT") {
            ...
            if ($type == "1") {
                ...
                $query = $this->db->query("select `m`.`sigle` AS `sigle`,`m`.`titre` AS `titre`,`u`.`idProgramme` AS `idProgramme`,`u`.`semestre` AS `semestre` from (`module` `m` join `unite` `u`) where ((`m`.`sigleunite` = `u`.`sigle`) and `m`.`sigle` in (select distinct `planetudes`.`sigle` from `planetudes` where ((`planetudes`.`annee` = $anneeCourante) and u.`idProgramme` = '" . $loadId . "' and  u.semestre = " . $semestre . " and (`planetudes`.`semestre` = $semestreCourant))))");
            } elseif ($type == "2") {
                $query = $this->db->query("SELECT m.* FROM `infomodule_element` m,groupe g where g.sigle=m.sigle and g.matriculeEmploye='" . $matriculeEmploye . "'");
                //$this->db->insert("r_events", array('test'=>"SELECT m.* FROM `infomodule_element` m,groupe g where g.sigle=m.sigle and g.matriculeEmploye='".$matriculeEmploye."'"));
            } elseif ($type == "3") {
...
echo $this->db->last_query();
    ...
    function getpvFraude($matricules, $semestre, $annee, $evaluation) {
        ...
        $data = array();
        if(empty($matricules) ||  empty($semestre) ||  empty($annee) ||  empty($evaluation)) // add by Med Bakar 24-09-2021
          return $data;
    ...
    function getNotezero($matricules, $semestre, $annee, $evaluation) {
        ...
        if(empty($matricules) || empty($semestre) || empty($annee) || empty($evaluation))
        return $str;
        $query = $this->db->query($str);
        if ($query->num_rows() > 0) {
...
///          ECHO $del;
    ...
    function ajouter_modif_note_temp($semestre, $sigle, $matriculeEtudiant, $note, $session, $annee) {
     ...
     function info_liste_modification_note($annee,$semestre,$session) {
      $info = array();
   $query = $this->db->query("select t.*,m.titre,u.semestre as sem  from notespartielles t ,module m,unite u  where m.sigle=t.sigle and u.sigle=m.sigleunite and t.annee=$annee and t.semestre=$semestre and t.idEvaluation=$session");
    //    $query = $this->db->query("select *  from notespartielles ");
        if ($query->num_rows() > 0) {
            foreach ($query->result_array() as $row) {
                $info[] = $row;
                //  $elements['anc_sigle'][] = $row['anc_sigle'];
...
//ajouter par MedBakar 02-04-2020    
    function list_etudiant_inscrit_pour_sigle($sigle,$annee){
       // $sql="select distinct matriculeEtudiant from planetudes where sigle='".$sigle."' and (annee=$annee and semestre=3) or (annee=($annee+1) and semestre=1)";
        //echo $sql;
        $data=array();
        $result=$this->db->query("select distinct matriculeEtudiant from planetudes where sigle='".$sigle."' and (annee=$annee and semestre=3) or (annee=($annee+1) and semestre=1)");
 ...
 //fin ajout MedBakar 10-04-2020
    ...
    function validation_modification_notes_eliminatoires($employe,$pass) {/*AZ*/
      ...
      function regle_passage($annee=' '){//add by MedBakar 10-09-2020
        ...
        $query = $this->db->query("SELECT * from regles_passage where DATE_FORMAT(date_debut, '%Y')<=$annee and (DATE_FORMAT(date_fin, '%Y')>=$annee or date_fin is null ) ");
          $regle =array();
//    echo $this->db->last_query();
          if ($query->num_rows() > 0) {
              foreach ($query->result_array() as $row) {
 ...
 * add by MedBakar
    ...
    function get_module_rattrapes_paire_new($liste,$niveau) {
        ...
        $sql = '';
        $sql = "SELECT  u.`sigle` as idModule,m.sigle as sigle, m.nbCredits as ects, m.titre,u.semestre as semestre FROM unite u,module m  where 
...
//        $an = $_POST['annee'];
      //  echo $niveau;
        ...
        $sql = '';
        $sql = 'SELECT u.`sigle` as idModule,m.`sigle` as sigle, m.nbCredits as ects, m.titre FROM unite u,programme p,module m  where 
                u.sigle=m.sigleunite  and (p.idProgramme = "' . $idProgramme . '" or p.idProgramme ="TC") and u.semestre =' .$semestre;
...
//echo $sql;
 ...
 //ajouter le 07/08/2020 zeinebou
...
fopen("ficher_excel_importer/".$_FILES['file']['name'],'w');
		...
		$sheetdata =$sheet->toArray();
			return $sheetdata;}
//08/08/2020  zeinebou
function import_to_table($table,$dataTable){
/* les donnees sont recuperer dans un tableau multid */

				$requete="insert into $table ( ";
				/*$querycol="SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='iup_19_08'.'$table'";
                                 echo $querycol; 
			if($table=='bac_mauritania'){ $col= "id";}
			if($table=="autorisation_e"){$querycol.="and COLUMN_NAME!='contacts'and COLUMN_NAME!='personne_ressource'and COLUMN_NAME!='prenomPere_fr'";}
			$res=$this->db->query($querycol);*/
				foreach($this->db->list_fields($table) as $row)
		                              {
                                    if($row!='id'&& $row!='contacts'&&$row!='personne_ressource'&&$row!='prenomPere_fr'){
                                    $requete.="$row,";}
			}		
			$requete=substr($requete, 0, strlen($requete) - 1);
				$requete.=") values";
				for($i=0;$i<=count($dataTable)-1;$i++){$requete.="(";
				$num_bac=$dataTable[$i]->num_bac;
				$requete.="$num_bac,";
				$annee=$dataTable[$i]->annee;
			    $requete.="$annee,";
					if($table=='autorisation_e'){
		            $nom=$dataTable[$i]->nom;
				$requete.="'$nom',";
				$prenom=$dataTable[$i]->prenom;
				$requete.="'$prenom',";
				
				$serie=$dataTable[$i]->serie;			
                $requete.="'$serie',";
                $idprogramme=$dataTable[$i]->idprogramme;
                $requete.="'$idprogramme',";
				$annne_autorisation=$dataTable[$i]->annee_autorisation	;		
					$requete.="$annne_autorisation,";}
				$cleImport=$this->get_cle_Import($table);
				 $requete.="'$cleImport',";
				 $dateImport=date("Y-m-d H-i-s");
                   $requete.="'$dateImport',";				
					
				$requete=substr($requete, 0, strlen($requete)-1);$requete.="),";
				}
				$requete=substr($requete, 0, strlen($requete)-1);
...
return $res;}
//14/08/2020 zeinebou
function get_cle_Import($table){
	$today=date("Ymd");
	$query="select max(cleImport) clemax from $table where cleImport like '$today%'";
		$res=$this->db->query($query);$max=$res->result();
		if($res->num_rows >0 && $max[0]->clemax!=0)
		{
			$cle=$max[0]->clemax;
			return $cle+1;
}else{ $cle=date("Ymd0");
...
//14/08/2020 zeinebou
function verifier_existence_lignes($table,$dataTable){
$requete="select * from $table where ";$requete.="(num_bac,annee)in(";
 $existeData=array();
				for($i=0;$i<=count($dataTable)-1;$i++)
				{$num_bac=$dataTable[$i]->num_bac;
			      $annee=$dataTable[$i]->annee;
					 $requete.="($num_bac,$annee),";
			}
			$requete=substr($requete, 0, strlen($requete) - 1);
			$requete.=")";
            $resul=$this->db->query($requete);
			$nbrlignes=$resul->num_rows();
			$row=$resul->result();
    for($h=0;$h<=$nbrlignes-1;$h++)
		{  
				  $existeData[$h]['num_bac']=$row[$h]->num_bac;
						$existeData[$h]['annee']=$row[$h]->annee;
						if($table=='autorisation_e'){
						$existeData[$h]['nom']=$row[$h]->nom;
						$existeData[$h]['prenom']=$row[$h]->prenom;
						$existeData[$h]['serie']=$row[$h]->serie;
						$existeData[$h]['idprogramme']=$row[$h]->idProgramme;
						$existeData[$h]['annee_autorisation']=$row[$h]->anneeAutorisation;	 }
						
			}	
		
$data=array('existeData'=>$existeData,'nbrlignes'=>$nbrlignes);
return $data;
}	//15/08/2020 zeinebou
function supprimer_donnees_existe($table,$donnees){
	$requete="delete from $table where (num_bac,annee)in(";
	for($i=0;$i<count($donnees);$i++){
		
$requete.="(";
$num_bac=$donnees[$i]->num_bac;
$requete.="$num_bac,";
...
// fin d'ajout par Zeinebou
    function update_note_afterValide() {
         ...
         print_r($_POST);
               ...
               $q1 = "update notespartielles set note='" . $_POST['note'] . "' where  matriculeEtudiant='" . $_POST['matriculeEtudiant'] . "' and idEvaluation=" . $_POST['idEvaluation'] . " and sigle='" . $_POST['sigle'] . "' and semestre=" . $_POST['semestre'] . " and annee=" . $_POST['annee'];
                    $this->db->query($q1);
                
            
}
//16/09/2021 znb modif de la fonction getInfoBulletinEtudiant_liste() convenable avec la conception ing
       function getInfoPV_liste($matriules,$semestre,$annee) {

        //$data['note'] et_informations_etudiant//
$maxprogramme="(select max(semestredebut) from dossieretudiant de,programme po where de.matriculeetudiant=e.matriculeetudiant and po.idprogramme=de.idprogramme)";
            
           $sql = 'select e.*,p.nom as programme,p.idprogramme as idprogramme  from etudiant e, dossieretudiant d,programme p where d.idProgramme= p.idProgramme and e.matriculeetudiant=d.matriculeetudiant and  e.matriculeetudiant in (' . $matriules . ') '.
                   'and p.semestredebut='.$maxprogramme.'order by matriculeEtudiant';
        // echo $sql;
        $infos = array();
        if(empty($matriules) ) // add by Med BAkar 24-09-2021
            return $infos;
    ...
    function get_etudiants_programme($programme){
            
            $requette="SELECT DISTINCT matriculeEtudiant FROM `dossieretudiant` where idProgramme='$programme'";
                   // . "`dossieretudiant`  where idProgramme='$programme'";
            $res= $this->db->query($requette);
           $matricules=array();
             if ($res->num_rows() > 0) {
            foreach ($res->result_array() as $row) {
                $matricules[]=$row['matriculeEtudiant'];
        ...
        return $matricules;}
        // add by medn
        function insert_list_etudiants($matriculeEtudiant,$idProgramme,$niveaup,$ECTS){
 ...
 //26-10-2021 addad by fatimetou
          function get_etudiants_matricules($programme,$annee,$semestre){
          $sem=$semestre;    
       if($semestre%2==0) {
               $semestre=1;
      ...
      //            $requette="SELECT DISTINCT matriculeEtudiant FROM `dossieretudiant` where idProgramme='$programme' and matriculeEtudiant".
        //              " in (select matriculeEtudiant from notespartielles where annee='$annee' and semestre='$semestre' )";
            
             $requette="SELECT DISTINCT matriculeEtudiant FROM `dossieretudiant` where idProgramme='$programme' and matriculeEtudiant".
                      " in (select matriculeEtudiant from notespartielles where annee='$annee' and semestre='$semestre' and"
                       ." sigle in (Select m.sigle from module m join unite u on(m.sigleUnite=u.sigle) and u.semestre=$sem))";
     ...
     //added by fatimetou 02-02-2022
         ...
         function list_matiere(){
             $sessioncourante =$this->get_session_courante();
           $anneeCourante = $sessioncourante['annee'][0];
           //echo $anneeCourante;
            $qeury=("SELECT  DISTINCT `sigle` FROM `groupe`");
            //$data[]=0;
         ...
         function list_groups($matiere){
             $sessioncourante =$this->get_session_courante();
           $anneeCourante = $sessioncourante['annee'][0];
           //echo $anneeCourante;
            $qeury=("SELECT DISTINCT (`idGroupe`)  FROM `groupe` WHERE `annee`=$anneeCourante AND `sigle`='$matiere'" );
          //  echo $qeury;
            //$data[]=0;
              $result= $this->db->query($qeury);
                foreach ($result->result_array() as $row){
        ...
        function verifier_existance_etuds_planetudes($dataTable){
$requete="select * from planetudes where ";$requete.="matriculeEtudiant in(";
 $existeData=array();
				for($i=0;$i<=count($dataTable)-1;$i++)
				{$matriculeEtudiant=$dataTable[$i]->matriculeEtudiant;
			      //$annee=$dataTable[$i]->annee;
					 $requete.="($matriculeEtudiant),";
			}
			$requete=substr($requete,0, strlen($requete) - 1);
			$requete.=")";
                       // echo $requete; 
    ...
    for($h=0;$h<=$nbrlignes-1;$h++)
		{  
				  $existeData[$h]['matriculeEtudiant']=$row[$h]->matriculeEtudiant;						
			}	
		
$data=array('existeData'=>$existeData,'nbrlignes'=>$nbrlignes);
                        if(empty( $existeData)){return false;}
                        else{return true;}

}
function verifier_existance_etudiant_listetudiant($dataTable,$idgroupe){
$requete="select * from `listeetudiants` WHERE `idGroupe`='$idgroupe' AND matriculeEtudiant in(";
 $existeData=array();
				for($i=0;$i<=count($dataTable)-1;$i++)
				{$matriculeEtudiant=$dataTable[$i]->matriculeEtudiant;
			      //$annee=$dataTable[$i]->annee;
...
$data=array('existeData'=>$existeData,'nbrlignes'=>$nbrlignes);
        ...
        function import_to_table_listetudiant($dataTable,$idgrp){
/* les donnees sont recuperer dans un tableau multid */
$matriculeEtudiant=0;
				$requete="INSERT INTO `listeetudiants` (matriculeEtudiant,idGroupe) VALUES";
				for($i=0;$i<=count($dataTable)-1;$i++){
                                    $requete.="(";
                                $matriculeEtudiant=$dataTable[$i]->matriculeEtudiant;
				$requete.="$matriculeEtudiant,'$idgrp'";					
				//$requete=substr($requete, 0, strlen($requete)-1);
                                $requete.="),";
				}
				$requete=substr($requete, 0, strlen($requete)-1);
...
function supprimer_donnees_existe_listetuds($donnees,$grp){
	$requete="delete from $table where (num_bac,annee)in(";
	for($i=0;$i<count($donnees);$i++){
...
return $res;
 ...
 //modification par fatimetou 14-02-2022
function get_etudiant_anne_semestre_mq_des($annee, $semestreEtu, $idProgramme, $semestre, $session, $tri) {// A revoir pour que �a marche avec les autrs ann�es
        $etudiants = array();
        $query = NULL;
 
         $query="SELECT DISTINCT p.matriculeEtudiant FROM planetudes p,module m,unite u WHERE u.sigle=m.sigleUnite"
                 . " and m.sigle=p.sigle and u.idProgramme='$idProgramme' and p.annee=$annee and p.semestre=$semestre and u.semestre=$semestreEtu and "
                 . "p.matriculeEtudiant in (SELECT DISTINCT(matriculeEtudiant) from planetudes where "
                 . "annee=$annee and sigle in(SELECT DISTINCT(sigle) FROM `module` WHERE semestreDesactivation<=$annee$semestre AND "
                 . "semestreDesactivation IS NOT null))";
          if($tri==1){
             $query.="ORDER by p.matriculeEtudiant";
         }
...
function lire_ficher_excel_etd(){
...
fopen("ficher_excel_importer/".$_FILES['file']['name'],'w');
    ...
    $sheetdata =$sheet->toArray();
      return $sheetdata;}
//04/02/2022  table etudiant
function import_to_table_etd($table1,$dataTable){
/* les donnees sont recuperer dans un tableau multid */
$table1='dossieretudiant';
    
        $requete="insert into $table1 (matriculeEtudiant,idProgramme,grade )";
        $requete.=" values";
        $requete=substr($requete, 0, strlen($requete) - 1);
        for($i=0;$i<count($dataTable);$i++){$requete.="(";
...
function import_to_table_aut($table,$dataTable){
/* les donnees sont recuperer dans un tableau multid */
			...
			$requete=substr($requete, 0, strlen($requete) - 1);
				...
				$requete.="'$contacts',";
                                $personne_ressource='';
				$requete.="'$personne_ressource',";
                                $prenomPere_fr='';
				$requete.="'$prenomPere_fr',";
				$annne_autorisation=date("Y");		
				$requete.="$annne_autorisation,";
				$cleImport=$this->get_cle_Import($table);
				 $requete.="$cleImport,";
				 $dateImport=date("Y-m-d H-i-s");
...
function supprimer_donnees_existe_etd($table1,$donnees){
  $requete="delete from $table1 where matriculeEtudiantin(";
  for($i=0;$i<count($donnees);$i++){
    
$requete.="(";
$matricule=$donnees[$i]->matriculeEtudiant;
$requete.="$matricule,";
...
//04/02/2022 dpssier
      ...
      function import_to_table_et($table,$dataTable){
/* les donnees sont recuperer dans un tableau multid */
...
foreach($this->db->list_fields($table) as $row)
                                    ...
                                    if( $row=='matriculeEtudiant'
                                            ...
                                            && $row=='telUrgence'
                                            && $row=='contactUrgence'
                                            && $row=='lienParenteUrgence'
                                            && $row=='raisonInactif'
                                            && $row=='actif'
                                            && $row=='nationalite'
                                            && $row=='photoEtudiant'
                                            && $row=='nomParents'
                                            && $row=='annee_inscription'
		...
		$matriculeEtudiant=$dataTable[$i]->matriculeEtudiant;
              ...
              $requete.="'$prenomArabe',";
               $surnom='';
              $requete.="'$surnom',";
              $sexe=$dataTable[$i]->sexe;
                $requete.="'$sexe',";
               $dateNaissance=$dataTable[$i]->dateNaissance;
              $dd= new DateTime($dateNaissance);
           
                $f= $dd->format('Y-m-d');
             
                $requete.="'$f',";
                $lieuNaissance=$dataTable[$i]->lieuNaissance;
                $requete.="'$lieuNaissance',";
                  $lieuNaissance_ar='';
                   $requete.="'$lieuNaissance_ar',";
                   $NIN=$dataTable[$i]->NIN;
                $requete.="'$NIN',";
                $infoBac=77;
                $requete.="$infoBac,";
                ...
                $requete.="$idAdresse,";
                $idAdresseParent=0;
                $requete.="$idAdresseParent,";
                
                 $telephone=$dataTable[$i]->telephone;
                $requete.="'$telephone',";
                $telephone2='';
                $requete.="'$telephone2',";
                $telParents='';
                ...
                $requete.="'$telUrgence',";
                $contactUrgence='';
                $requete.="'$contactUrgence',";
                $lienParenteUrgence='';
                $requete.="'$lienParenteUrgence',";
                $raisonInactif='';
                $requete.="'$raisonInactif',";
                $actif=1;
                $requete.="$actif,";
                 $nationalite='';
                $requete.="'$nationalite',";
                 $photoEtudiant='';
                $requete.="'$photoEtudiant',";
               	$nomParents='';
                $requete.="'$nomParents',";
                $annee_inscription=date("Y");
                $requete.="$annee_inscription,";
                $created_by='';
                $requete.="'$created_by',";
                $dateInscription=date("Y-m-d H-i-s");
                $requete.="'$dateInscription',";
                 $prenomPere_fr='';
                $requete.="'$prenomPere_fr',";
		 $prenomPere_ar='';
                $requete.="'$prenomPere_ar',";
...
return $res;}
 function verifier_existence_lignes_etd($table1,$dataTable){
    

$requete="select matriculeEtudiant from $table1 where  matriculeEtudiant in(";
 $existeData=array();
				for($i=0;$i<=count($dataTable)-1;$i++)
				{
                                    $matriculeEtudiant=$dataTable[$i]->matriculeEtudiant;
...
function verifier_existence_lignes_aut($table2,$dataTable){
$requete="select * from $table2 where ";$requete.="num_bac in(";
 $existeData=array();
				for($i=0;$i<=count($dataTable)-1;$i++)
				{$num_bac=$dataTable[$i]->num_bac;
...
function supprimer_donnees_existe_aut($table2,$donnees){
	$requete="delete from $table2 where num_bac in(";
	for($i=0;$i<count($donnees);$i++){
...
function verifier_existence_lignes_et($table,$dataTable){
    
$requete="select * from $table where matriculeEtudiant in(";
 $existeData=array();
			for($i=0;$i<=count($dataTable)-1;$i++)
				{
                                  $matriculeEtudiant=$dataTable[$i]->matriculeEtudiant;
...
$data=array('existeData'=>$existeData,'nbrlignes'=>0);
return $data;
}
function supprimer_donnees_existe_et($table,$donnees){
  $requete="delete from $table where matriculeEtudiantin(";
  for($i=0;$i<count($donnees);$i++){
...
return $res;}//15/08/20
function inserer_etudiant_bac($post) {
        $num_bac = $post['num_bac'];
        $annee=$post['annee'];
        $data =array();
      $compteurMessage =0;
      $compteursValide=0;
    if (isset($post['num_bac'])) {     
        $query = $this->db->query("SELECT DISTINCT `num_bac` FROM `bac_mauritania` WHERE `num_bac`=".$num_bac);
   
             if ( $query->num_rows() > 0 ) {
               $data['message'][$compteurMessage] = 'L\'etudiant <b>' . $num_bac . ' </b>est deja inserer';
                $compteurMessage++; 
    } 
        else{
                $info = array(
	           ...
	           'dateImport'=>date("Y-m-d H-i-s")
                ...
                $this->db->insert('bac_mauritania', $info);
                      
                      $data['valide'][$compteursValide]='L\'etudiant <b>' . $num_bac .'</b>est bien  inserer  </b>';
                     $compteursValide++;
            }
     ...
     function get_module_filiere_semstre($semstre,$filiere){
        $sig=array();
        $str="select sigle from infomodule_element where filiere='$filiere' and semestre=$semstre";
       $query=$this->db->query($str);
       if($query->num_rows()>0){
           foreach($query->result_array() as $row){
               $sig[]=$row['sigle'];
     ...
     function get_module_GEER(){
        $sig=array();
        $str="select sigle from infomodule_element where filiere='TC' and semestre=1";
       $query=$this->db->query($str);
       if($query->num_rows()>0){
           foreach($query->result_array() as $row){
               $sig[]=$row['sigle'];
...
function Insecrire($post) {
        $Matricule=$post['matriculeEtudiant'];
        $nom =$post['nom'];
            $prenom=$post['prenom'];
        $idProgramme=$post['idProgramme'];
        $annee=$post['annee'];
        $num_bac=$post['num_bac'];
        $serie=$post['serie'];
         $nomArabe=$post['nomArabe'];
         $nationalite=$post['nationalite'];
          $prenomArabe=$post['prenomArabe'];
           $dateNaissance= $post['year'] . '-' . $post['month'] . '-' . $post['day'];
           $lieuNaissance=$post['lieuNaissance'];
          $NIN=$post['NIN'];
           $sexe=$post['sexe'];     
         $anneeAutorisation=$post['anneeAutorisation'];
        $data =array();
        $sessionCourante =$this->get_session_courante();
           $anneeCourante = $sessionCourante['annee'][0];
           $semestreCourant = $sessionCourante['semestre'][0];
        ...
        $nombreSigle=0;
         if($idProgramme=='TC'){
            $tableSigle=$this->get_module_GCGP();
         $nombreSigle=count($tableSigle); } 
       /* if($idProgramme=='TC'){
            $tableSigle=$this->get_module_GEER();
         $nombreSigle=count($tableSigle); } */
    if(isset($Matricule)) {     
       $query = $this->db->query("SELECT DISTINCT `matriculeEtudiant` FROM `etudiant` WHERE `matriculeEtudiant`='$Matricule'");

        $query = $this->db->query("SELECT DISTINCT `matriculeEtudiant` FROM `dossieretudiant` WHERE `matriculeEtudiant`='$Matricule'");
    for($s = 0; $s < $nombreSigle; $s++){     
            $this->db->where(array('matriculeEtudiant' =>$Matricule, 'annee' => $anneeCourante, 'semestre' => $semestreCourant, 'sigle' =>$tableSigle[$s]));
    $query1 = $this->db->get('planetudes');}
           ...
           $d=date("Y-m-d H-i-s");
             $data = array(
                ...
                'idProgramme' =>$idProgramme,
                 'anneeAutorisation' => $anneeAutorisation,
...
$this->db->query("INSERT INTO `etudiant` (`matriculeEtudiant`, `nom`, `nomArabe`, `prenom`, "
        ...
        . "`prenomPere_ar`) VALUES ($Matricule, '$nom', '$nomArabe', '$prenom', '$prenomArabe', "
        . "NULL, '$sexe', '$dateNaissance', '$lieuNaissance', '', '$NIN', '0', NULL, NULL, '0',"
        . " NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL,"
        . " '1', '$nationalite', '', NULL, '2020', '', '$d', '', '')");          
     ...
     public function importData_new_etudiant($data) {
 
        $res = $this->db->insert_batch('etudiant',$data);
        if($res){
            return TRUE;
        }else{
            return FALSE;
...
//add by team scolarite 22-23
public function get_etudiant_progression($programme,$niveau){
  $etudiant = array();
$sessionCourante =$this->get_session_courante();
           $anneeCourante = $sessionCourante['annee'][0];
           $semestreCourant = $sessionCourante['semestre'][0];
$session="(3,";
           ...
           if( $semestreCourant==3){
               $anneeCourante=$anneeCourante;
$session.="$anneeCourante),(1,".($anneeCourante+1).")";

//            $session.="$anneecourante),(1,".$anneeCourante+1.")";
            }
            if($semestreCourant==1){
$session.="".($anneeCourante-1)."),(1,".($anneeCourante).")";
  
                $anneeCourante=$anneeCourante+1;
            }
      $prg="";
      if(strcmp($programme,'tous')!== 0){
       $prg="e.idProgramme='".$programme."' and ";
}
        $query = $this->db->query("SELECT DISTINCT p.matriculeEtudiant , e.idProgramme FROM `planetudes` p join dossieretudiant e join module m join unite u ".
" on(e.matriculeEtudiant=p.matriculeEtudiant and m.sigleunite=u.sigle and p.sigle=m.sigle) where u.semestre + 1 < $niveau and (p.semestre,p.annee) in(".$session.")"
."and e.idProgramme!='TC' and ".$prg." EXISTS (SELECT DISTINCT p1.matriculeEtudiant,m.sigle FROM `planetudes` p1 join module m join unite u1 "
."on(m.sigleunite=u1.sigle and p1.sigle=m.sigle) where u1.semestre + 1 >= $niveau and (p.semestre,p.annee) in(".$session.")"
."and p.matriculeEtudiant=p1.matriculeEtudiant) order by 1;");
$q="SELECT DISTINCT p.matriculeEtudiant , e.idProgramme FROM `planetudes` p join dossieretudiant e join module m join unite u ".
" on(e.matriculeEtudiant=p.matriculeEtudiant and m.sigleunite=u.sigle and p.sigle=m.sigle) where u.semestre + 1 < $niveau and (p.semestre,p.annee) in(".$session.")"
."and e.idProgramme!='TC' and ".$prg." EXISTS (SELECT DISTINCT p1.matriculeEtudiant,m.sigle FROM `planetudes` p1 join module m join unite u1 "
."on(m.sigleunite=u1.sigle and p1.sigle=m.sigle) where u1.semestre + 1 >= $niveau and (p.semestre,p.annee) in(".$session.")"
."and p.matriculeEtudiant=p1.matriculeEtudiant) order by 1;";
...
//and e.matriculeEtudiant not in (SELECT `matriculeEtudiant` from dossieretudiant WHERE `idProgramme`!='TC') order by `matriculeEtudiant`;");
        if ($query->num_rows() > 0) {
            foreach ($query->result_array() as $row) {
                $etudiant['matricule'][] = $row['matriculeEtudiant'];
               $etudiant['programme'][]=$row['idProgramme'];
            }
        }
        return $etudiant;
